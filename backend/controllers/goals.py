...
def read_goal(goalid: int, db: Session = Depends(get_postgres_db)):
...
def update_goals(goalid: int, goal_update: GoalsUpdate, db: Session = Depends(get_postgres_db)):
...
def read_goals_history(db: Session = Depends(get_postgres_db)):
    return get_all_goals_history(db)

@router.get("/api/action", response_model=list[ActionResponse])
def read_action(db: Session = Depends(get_postgres_db)):
    return get_action(db)

# p
@router.get("/api/p", response_model=list[PResponse])
def read_p(db: Session = Depends(get_postgres_db)):
    return get_all_p(db)

@router.get("/api/p/{id}", response_model=PResponse)
def read_p(id: int, db: Session = Depends(get_postgres_db)):
...
# def create_p_endpoint(p: PCreate, db: Session = Depends(get_postgres_db)):
...
def update_p_endpoint(id: int, p: PUpdate, db: Session = Depends(get_postgres_db)):
    print("status type:",p) 
...
def read_status(db: Session = Depends(get_postgres_db)):
    return get_all_status(db)

# @router.post("/api/status", response_model=StatusResponse)
# def create_status(status: StatusCreate, db: Session = Depends(get_postgres_db)):
...
def read_status(id: int, db: Session = Depends(get_postgres_db)):
...
def update_status_route(id: int, status_update: StatusUpdate, db: Session = Depends(get_postgres_db)):
...
def read_b(db: Session = Depends(get_postgres_db)):
    return get_all_b(db)

# @router.post("/api/b", response_model=BResponse)
# def create_B(b: BCreate, db: Session = Depends(get_postgres_db)):
...
def read_b(id: int, db: Session = Depends(get_postgres_db)):
...
def update_b_endpoint(id: int, b: BUpdate, db: Session = Depends(get_postgres_db)):
...
def read_e(db: Session = Depends(get_postgres_db)):
    return get_all_e(db)

# @router.post("/api/e", response_model=EResponse)
# def create_e_endpoint(e: ECreate, db: Session = Depends(get_postgres_db)):
...
def read_(id: int, db: Session = Depends(get_postgres_db)):
...
def update_e_endpoint(id: int, e: PUpdate, db: Session = Depends(get_postgres_db)):
...
def read_d(db: Session = Depends(get_postgres_db)):
    return get_all_d(db)

# @router.post("/api/d", response_model=DResponse)
# def create_d(D: DCreate, db: Session = Depends(get_postgres_db)):
...
def read_D(id: int, db: Session = Depends(get_postgres_db)):
...
def update_D_endpoint(id: int, d: DUpdate, db: Session = Depends(get_postgres_db)):
...
def read_proj(db: Session = Depends(get_postgres_db)):
    return get_all_proj(db)

@router.get("/api/proj/{id}", response_model=ProjResponse)
def read_proj(id: int, db: Session = Depends(get_postgres_db)):
...
def create_proj_endpoint(proj: ProjCreate, db: Session = Depends(get_postgres_db)):
    try:
...
def update_proj_endpoint(id: int, proj: ProjUpdate, db: Session = Depends(get_postgres_db)):
...
def read_role(db: Session = Depends(get_postgres_db)):
    return get_all_role(db)

@router.get("/api/role/{id}", response_model=RoleResponse)
def read_role(id: int, db: Session = Depends(get_postgres_db)):
...
def create_role_endpoint(role: RoleCreate, db: Session = Depends(get_postgres_db)):
    try:
...
def update_role_endpoint(id: int, role: RoleUpdate, db: Session = Depends(get_postgres_db)):
...
# def create_roleMaster_endpoint(role: RoleMasterCreate, db: Session = Depends(get_postgres_db)):
...
def update_role_endpoint(id: int, role: RoleMasterUpdate, db: Session = Depends(get_postgres_db)):
...
def read_RoleMaster_By_Id(email: str, db: Session = Depends(get_postgres_db)):
    print("email",email)
...
def get_who_and_vp_by_email(email: str, db: Session = Depends(get_postgres_db)):
...
def get_global_clone_setting(db: Session = Depends(get_postgres_db)):
    result = db.execute(
