...
@router.post("/goalslist/", response_model=GoalsResponse)
def create_goals(goal: Goals, db: Session = Depends(get_db)):
    return create_goal(db=db, goal=goal)

@router.get("/goalslist/{goalid}", response_model=GoalsResponse)
...
@router.get("/goalslist/", response_model= list[GoalsResponse])
def read_goals(db: Session = Depends(get_db)):
    return get_all_goals(db)

@router.put("/goalslist/{goalid}", response_model=GoalsUpdate)
