...
@router.get("/goals/{goalid}", response_model=GoalsResponse)
...
@router.get("/goals/", response_model= list[GoalsResponse])
def read_goals(db: Session = Depends(get_db)):
    return get_all_goals(db)

@router.put("/goals/{goalid}", response_model=GoalsUpdate)
